...
- 玩家人數：2 - 4 人
- 棋盤大小：公式為 `(玩家數 * 5 + 1) ^ 2`，例如：2人為 11x11，4人為 21x21
- 遊戲目標：不斷連結並撐到最後
...
以此符號附近的四個方位擇一，不限距離長度，忽略[死亡符號](#死亡符號)，選擇與此符號距離最近的[可為對象符號](#可為對象符號)進行[攻擊](#攻擊)。
發動條件：不為[失聯符號](#失聯符號)、[死亡符號](#死亡符號)，附近八個方位不可以有[可為對象符號](#可為對象符號)。
形象設定：狙擊符號不願意受到別的符號打擾。

### 掩體符號
形象設定：掩體符號自願成為肉盾，沒有意識。
...
同理，`F` 若沒有符號時，於 `C` 的炸彈符號將會對 `FL` 與 `FR` 進行[攻擊](#攻擊)。
...
不可作為連線來源的符號為：[失聯符號](#失聯符號)、[掩體符號](#掩體符號)
